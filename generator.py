...
class Tournament():
    ...
    def _outputJScript(self, f):
        f.write("<script>\n")
        f.write("function replaceText(key, params) {\n")
        f.write("  let param = params[key];\n")
        f.write("  if (param) {\n")
        f.write("    const elems = document.getElementsByName(key);\n")
        f.write("    for(let x=0; x < elems.length; x++) {\n")
        f.write("      elems[x].innerHTML = elems[x].innerHTML.replace(key, unescape(param));\n")
        f.write("    }\n")
        f.write("  }\n")
        f.write("}\n")
        f.write("function getParams() {\n")
        f.write("  let idx = document.URL.indexOf('?');\n")
        f.write("  let params = new Array();\n")
        f.write("  if (idx != -1) {\n")
        f.write("    let pairs = document.URL.substring(idx+1, document.URL.length).split('&');\n")
        f.write("    for (let i=0; i<pairs.length; i++) {\n")
        f.write("      nameVal = pairs[i].split('=');\n")
        f.write("      params[nameVal[0]] = nameVal[1];\n")
        f.write("    }\n")
        f.write("  }\n")
        f.write("  return params;\n")
        f.write("}\n")
        f.write("params = getParams();\n")
        f.write("for (let i = 1; i <= {}; i++) {{\n".format(len(self._teams)))
        f.write("  replaceText(\"Team\"+i.toString(), params);\n")
        f.write("}\n")
        f.write("for (let i = 1; i <= {}; i++) {{\n".format(len(self._games)))
        f.write("  replaceText(\"Game\"+i.toString(), params);\n")
        f.write("}\n")
        f.write("</script>\n")
        return
